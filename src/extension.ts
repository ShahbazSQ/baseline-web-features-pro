...
interface BaselineFeature {
  ...
  description?: string;
  status: {
    baseline_status?: 'widely' | 'newly' | 'limited' | false | boolean;
...
class BaselineAnalysisEngine {
  private features: Map<string, BaselineFeature> = new Map();
...
private async fetchLiveBaselineData() {
  try {
      ...
      if (this.detectionPatterns.has(featureId)) {
        ...
        if (existingFeature && data.status) {
          ...
          existingFeature.status = {
            baseline_status: baselineStatus as any,
    ...
    const fallbackFeatures = [
      {
        ...
        patterns: [/\?\./g],
        status: { 
          baseline_status: 'widely' as const, 
      ...
      },
      {
        ...
        patterns: [/\?\?/g],
        status: { baseline_status: 'widely' as const, baseline_low_date: '2020-04-21' },
        support: {
                  chrome: '80',
                  firefox: '72',
                  safari: '13.1',
      ...
      },
      {
        ...
        status: { baseline_status: 'widely' as const, baseline_low_date: '2017-09-05' },
          support: {
                    chrome: '55',
                    firefox: '52',
                    safari: '10.1',
      ...
      },
      {
        ...
        name: 'Fetch API',
        patterns: [/fetch\s*\(/g],
        status: { baseline_status: 'widely' as const, baseline_low_date: '2017-03-07' },
        support: {
                chrome: '42',
                firefox: '39',
                safari: '10.1',
      ...
      },
      {
        ...
        status: { baseline_status: 'widely' as const, baseline_low_date: '2017-03-16' },
        support: {
                chrome: '57',
                firefox: '52',
                safari: '10.1',
      ...
      },
      {
        ...
        status: { baseline_status: 'widely' as const, baseline_low_date: '2015-09-13' },
      ...
      },
      {
        ...
        status: { baseline_status: 'widely' as const, baseline_low_date: '2018-01-29' },
      ...
      },
      {
        ...
        status: { baseline_status: 'newly' as const, baseline_low_date: '2023-02-14' },
      ...
      },
      {
        ...
        patterns: [/@layer\s/g],
        status: { baseline_status: 'newly' as const, baseline_low_date: '2022-03-14' },
      ...
      },
      {
        ...
        status: { baseline_status: 'widely' as const, baseline_low_date: '2020-01-15' },
      ...
      },
      {
        ...
        status: { baseline_status: 'widely' as const, baseline_low_date: '2019-09-10' },
      ...
      },
      {
        ...
        patterns: [/:has\(/g],
        status: { baseline_status: 'newly' as const, baseline_low_date: '2023-12-15' },
      ...
      },
      {
        ...
        patterns: [/:is\(/g],
        status: { baseline_status: 'widely' as const, baseline_low_date: '2021-04-20' },
      ...
      },
      {
        ...
        status: { baseline_status: 'widely' as const, baseline_low_date: '2021-10-05' },
      ...
      },
      {
        ...
        status: { baseline_status: 'widely' as const, baseline_low_date: '2020-09-15' },
      ...
      },
      {
        ...
        status: { baseline_status: 'widely' as const, baseline_low_date: '2015-07-29' },
      ...
      },
      {
        ...
        status: { baseline_status: 'newly' as const, baseline_low_date: '2023-09-11' },
      ...
      },
      {
        ...
        status: { baseline_status: 'widely' as const, baseline_low_date: '2022-03-14' },
  ...
  private getSeverity(feature: BaselineFeature): 'info' | 'warning' | 'error' {
    if (feature.status.baseline_status === 'widely') return 'info';
    if (feature.status.baseline_status === 'newly') return 'warning';
    if (feature.status.baseline_status === 'limited') return 'warning';
    return 'error';
  }

  private getRecommendation(featureId: string, matchedText: string): string {
  ...
  calculateProjectStats(results: DetectionResult[]): ProjectStats {
    let totalFeatures = 0;
    ...
    results.forEach(result => {
      ...
      result.matches.forEach(match => {
        ...
        if (status === 'widely') widelySupported++;
        else if (status === 'newly') newlySupported++;
        else if (status === 'limited') limitedSupport++;
        else notBaseline++;
...
class BaselineTreeProvider implements vscode.TreeDataProvider<FeatureNode> {
  private _onDidChangeTreeData = new vscode.EventEmitter<FeatureNode | undefined | null | void>();
  readonly onDidChangeTreeData = this._onDidChangeTreeData.event;

  private results: DetectionResult[] = [];

  refresh(results: DetectionResult[]): void {
  ...
  private getCategoryNodes(): FeatureNode[] {
    const widelySupported: FeatureNode[] = [];
    const newlySupported: FeatureNode[] = [];
    const issues: FeatureNode[] = [];
...
interface FeatureNode {
  label: string;
  type: 'category' | 'feature' | 'info';
  status: string | boolean
  tooltip?: string;
  children?: FeatureNode[];
}

class BrowserSupportProvider implements vscode.TreeDataProvider<BrowserNode> {
  private _onDidChangeTreeData = new vscode.EventEmitter<BrowserNode | undefined | null | void>();
  readonly onDidChangeTreeData = this._onDidChangeTreeData.event;
  ...
  private groupByBrowser(): BrowserNode[] {
    const chromeFeatures: string[] = [];
    const firefoxFeatures: string[] = [];
    const safariFeatures: string[] = [];
    const edgeFeatures: string[] = [];
...
class ScoreProvider implements vscode.TreeDataProvider<string> {
  ...
  refresh(stats: ProjectStats): void {
    this.stats = stats;
...
class RecommendationsProvider implements vscode.TreeDataProvider<any> {
  ...
  private generateRecommendations(results: DetectionResult[]): string[] {
    const recs: string[] = [];
...
const scoreTreeView = vscode.window.createTreeView('baselineScore', {
  treeDataProvider: scoreProvider,  // Changed!
  showCollapseAll: false
});

const recommendationsTreeView = vscode.window.createTreeView('recommendations', {
  treeDataProvider: recommendationsProvider,  // Use recommendationsProvider, not treeProvider
  showCollapseAll: false
});
    ...
    vscode.window.withProgress({
      ...
      results.forEach(result => {
        result.matches.forEach(match => {
          ...
          if (match.severity === 'warning') severity = vscode.DiagnosticSeverity.Warning;
          if (match.severity === 'error') severity = vscode.DiagnosticSeverity.Error;
...
${result.matches[0]?.recommendation ? `- **Recommendation**: ${result.matches[0].recommendation}` : ''}
`).join('')}
...
`;
  ...
  // Register commands and providers
   context.subscriptions.push(
    ...
    featureTreeView,
    scoreTreeView,
    recommendationsTreeView,
    browserTreeView,
    hoverProvider, // ADD THIS LINE
    recommendationsTreeView,
    statusBarItem
  );
